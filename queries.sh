...
echo "$($PSQL "SELECT wt.name FROM games LEFT JOIN teams wt ON games.winner_id = wt.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT ot.name FROM games LEFT JOIN teams ot ON games.opponent_id = ot.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
